...
def test_domain_registry_contains_only_approved_ids_and_slugs() -> None:
    ...
    assert set().union(*(domain.keys() for domain in domains)) == {"id", "slug"}
    assert domains[0] == {"id": "NT-DA-0001", "slug": "human-nature"}
    assert domains[-1] == {"id": "NT-DA-0020", "slug": "ethics"}
    assert len({domain["id"] for domain in domains}) == 20
    assert len({domain["slug"] for domain in domains}) == 20
