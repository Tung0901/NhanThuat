...
def test_hiring_status_is_ready_for_review() -> None:
    status = load_document(ROOT / "docs" / "domains" / "hiring" / "status.yaml")

    assert status["domain_area_id"] == "NT-DA-0005"
    assert status["slug"] == "hiring"
...
def test_hiring_blueprint_files_exist() -> None:
    docs = ROOT / "docs" / "domains" / "hiring"
...
def test_hiring_relations_validate() -> None:
